...
export default async function Basket(req, res) {
  ...
  switch (req.method) {
    case "POST": {
      ...
      const isDuplicate = user.basket.some((p) => p.id == product.id);

      if (isDuplicate) {
        return res
          .status(409)
          .json({ message: "در سبد خرید موجود است !!", success: false });
      }

      jsonFileContent.users.forEach((u) => {
        if (u.id == user.id) {
          u.basket.push(product);
        }
      });
      ...
      return res
        .status(201)
        .json({ message: "به سبد خرید اضافه شد", success: true });
    }
    ...
    case "DELETE": {
      ...
      user.basket = user.basket.filter((p) => p.id != req.query.productId);
      jsonFileContent.users[userIndex] = user;
      fs.writeFileSync(jsonFilePath, JSON.stringify(jsonFileContent));
      return res
        .status(200)
        .json({ message: "از سبد خرید حذف شد", success: true });
    }
    default: {
      return res
        .status(400)
